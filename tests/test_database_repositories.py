...
from db.repositories import RevisionRepository, SessionRepository, TopicRepository, create_user
...
def test_foreign_key_constraint_prevents_orphan_topic(session) -> None:
    ...
    with pytest.raises(IntegrityError):
        topic_repo.create_topic(subject_id="missing-subject", name="Impossible Topic")
...
def test_session_repository_tracks_study_sessions(session) -> None:
    user = create_user(session)
    ...
    listed_sessions = session_repo.list_sessions_for_user(user.id)
